...
interface IComponentUnitProps {
  component: IAsset;
}

const BoltSvg = ({ statusColor }: { statusColor: string }) => {
  return (
    <svg
      width="9"
      height="12"
...
const ComponentUnit: React.FC<IComponentUnitProps> = (
  props: IComponentUnitProps
) => {
  const statusColor =
    props.component.status === "operating"
      ? "var(--primary-green-color)"
      : "var(--primary-red-color)";
  const sensorIcon = {
    energy: () => <BoltSvg statusColor={statusColor} />,
    vibration: () => (
      <div
        style={{
          width: 8,
  ...
  return (
    ...
    >
      ...
      {sensorIcon[props.component.sensorType!]()}
    </div>
  );
};
